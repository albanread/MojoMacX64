...
- **S4 — AIR compatibility.** Compare AIR emitted by KGEN's bitcode writers against the **verified reference** from S1's offline leg (Xcode 15.2 `metal`, triple `air64-apple-macosx14.2.0`, `air.*` metadata schema — loads and builds pipelines on the Vega II). Diff with `xcrun air-objdump` / `metal-readobj`; validate `+metal3_2,+air2_7_0` or pick the right level. Note: Xcode 15.2's offline MSL tops out at 3.1; the OS runtime accepts 3.2 — the trio targets the OS loader.
