...
# ===----------------------------------------------------------------------=== #
# Modular's first GPU example (max/examples/gpu-functions), carried into
# this fork's collection unmodified -- including DeviceContext() with no
# arguments, which resolves to the Apple GPU here. The canonical first
# kernel: one thread per element, TileTensor over device buffers.
