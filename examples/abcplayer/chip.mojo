...
# import. chip.mojo depends only on the standard library, so this single file
# is the whole dependency. If the synth engine in examples/chip/chip.mojo
...
fn chip_new() -> P:
    ...
    let st = external_call["calloc", P](Int(TOTAL_SLOTS), Int(8))
    put(st, S_VOL, 15)
