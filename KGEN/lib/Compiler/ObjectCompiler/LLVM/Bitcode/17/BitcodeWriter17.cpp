...
class ModuleBitcodeWriter : public BitcodeWriterBase {
  ...
  M::KGEN::PointerRewriter::PointerTypeMap PointerMap;
...
public:
  ...
  ModuleBitcodeWriter(const Module &M, StringTableBuilder &StrtabBuilder,
    ...
    PointerMap = M::KGEN::PointerRewriter::buildPointerMap(M);
...
public:
  ...
  IndexBitcodeWriter(BitstreamWriter &Stream, StringTableBuilder &StrtabBuilder,
    ...
    forEachSummary(
        [&](GVInfo I) { GUIDToValueIdMap[I.first] = ++GlobalValueId; });
  }
...
static uint64_t getOptimizationFlags(const Value *V) {
  ...
  // Skip flags for compare instructions to avoid LLVM 14 compatibility issues
  // The main issue is the 'samesign' flag on ICmp instructions
  if (isa<CmpInst>(V)) {
    // For LLVM 14 compatibility, don't include any compare instruction flags
    // The samesign flag and other newer compare flags cause "Invalid record"
    // errors
    return 0;
  }
...
void ModuleBitcodeWriter::writeValueAsMetadata(
  ...
  if (Function *F = dyn_cast<Function>(V)) {
    // Check if we have a rewritten type in PointerMap
    if (auto RewrittenTy = PointerMap.lookup(F))
      Ty = RewrittenTy;
    else
...
void ModuleBitcodeWriter::writeDIFile(const DIFile *N,
  ...
  if (N->getRawChecksum()) {
    ...
    // Maintain backwards compatibility with the old internal representation
    // of CSK_None in ChecksumKind by writing nulls here when Checksum is
    // None.
    Record.push_back(0);
...
// Generates an enum to use as an index in the Abbrev array of Metadata
// record.
...
void ModuleBitcodeWriter::writeModuleMetadata() {
  ...
  (void)OffsetAbbrev; // Suppress unused variable warning

  Abbv = std::make_shared<BitCodeAbbrev>();
  Abbv->Add(BitCodeAbbrevOp(bitc::METADATA_INDEX));
  Abbv->Add(BitCodeAbbrevOp(BitCodeAbbrevOp::Array));
  Abbv->Add(BitCodeAbbrevOp(BitCodeAbbrevOp::VBR, 6));
  Stream.EmitAbbrev(std::move(Abbv)); // IndexAbbrev not used
  ...
  // fixed size from the current bit number to get the location to backpatch.
  Stream.GetCurrentBitNo(); // IndexOffsetRecordBitPos not used
  ...
  // FIXME: Only store metadata for declarations here, and move data for
  // global variable definitions to a separate block (PR28134).
  for (const GlobalVariable &GV : M.globals())
    if (GV.hasMetadata())
...
void ModuleBitcodeWriter::writeConstants(unsigned FirstVal, unsigned LastVal,
  ...
  for (unsigned i = FirstVal; i != LastVal; ++i) {
    ...
    // Use corrected Metal kernel argument types for i64→i32 conversions
    Type *CurrentTy = VE.getMetalKernelArgType(V);
    if (CurrentTy != LastTy) {
      LastTy = CurrentTy;
    ...
    if (const ConstantInt *IV = dyn_cast<ConstantInt>(C)) {
      ...
      // XXX: how do support, e.g., `@global = [1 x ptr] [ptr @fun]`? we'd
      // need
      //      bitcast CEs, e.g., `[i8* bitcast (void ()* @fun to i8*)]`...
      if (LastTy->isPtrOrPtrVectorTy() ||
          (LastTy->isArrayTy() &&
           LastTy->getArrayElementType()->isPtrOrPtrVectorTy()))
        llvm_unreachable("pointers in constant aggregates are not yet "
      ...
      if (LastTy->isPtrOrPtrVectorTy() ||
          (LastTy->isArrayTy() &&
           LastTy->getArrayElementType()->isPtrOrPtrVectorTy()))
        llvm_unreachable("pointers in constant expressions are not supported "
                         "by the IR downgrader");
      ...
      default:
        if (Instruction::isCast(CE->getOpcode())) {
          ...
          // Use corrected Metal kernel argument types for i64→i32 conversions
          Record.push_back(
              VE.getTypeID(VE.getMetalKernelArgType(C->getOperand(0))));
      ...
      case Instruction::GetElementPtr: {
        ...
        for (const Value *Op : CE->operands()) {
          // Use corrected Metal kernel argument types for i64→i32 conversions
          Record.push_back(VE.getTypeID(VE.getMetalKernelArgType(Op)));
          Record.push_back(VE.getValueID(Op));
        }
        break;
      }
      ...
      case Instruction::ExtractElement:
        ...
        Record.push_back(VE.getValueID(C->getOperand(0)));
        Record.push_back(
            VE.getTypeID(VE.getMetalKernelArgType(C->getOperand(1))));
        Record.push_back(VE.getValueID(C->getOperand(1)));
        break;
      case Instruction::InsertElement:
        ...
        // Use corrected Metal kernel argument types for i64→i32 conversions
        Record.push_back(
            VE.getTypeID(VE.getMetalKernelArgType(C->getOperand(2))));
        Record.push_back(VE.getValueID(C->getOperand(2)));
        break;
      case Instruction::ShuffleVector:
        ...
        // Use corrected Metal kernel argument types for comparison and encoding
        Type *ResultType = VE.getMetalKernelArgType(C);
        Type *OperandType = VE.getMetalKernelArgType(C->getOperand(0));
        if (ResultType == OperandType) {
          Code = bitc::CST_CODE_CE_SHUFFLEVEC;
        } else {
          Code = bitc::CST_CODE_CE_SHUFVEC_EX;
          Record.push_back(VE.getTypeID(OperandType));
        }
        Record.push_back(VE.getValueID(C->getOperand(0)));
        Record.push_back(VE.getValueID(C->getOperand(1)));
...
bool ModuleBitcodeWriter::pushValueAndType(const Value *V, unsigned InstID,
  ...
  // Make encoding relative to the InstID using signed arithmetic.
  int32_t diff = (int32_t)InstID - (int32_t)ValID;
  Vals.push_back((unsigned)diff);

  if (ValID >= InstID) {
    Type *OpTy = Ty ? Ty : VE.getMetalKernelArgType(V);
    if (PointerMap.find(V) != PointerMap.end()) {
      OpTy = PointerMap[V];
    }

    Vals.push_back(VE.getTypeID(OpTy));
    return true;
  }
  return false;
}
...
bool ModuleBitcodeWriter::pushValueOrMetadata(const Value *V, unsigned InstID,
  ...
  return pushValueAndType(V, InstID, Vals, VE.getMetalKernelArgType(V));
}
...
void ModuleBitcodeWriter::pushValue(const Value *V, unsigned InstID,
  ...
  // Use signed arithmetic to handle backward references properly
  int32_t diff = (int32_t)InstID - (int32_t)ValID;

  // Encode as unsigned but with proper sign handling
  Vals.push_back((unsigned)diff);
}
...
void ModuleBitcodeWriter::writeInstruction(const Instruction &I,
  ...
  default:
    if (Instruction::isCast(I.getOpcode())) {
      ...
      // Use corrected Metal kernel argument types for i64→i32 conversions
      Type *Ty = VE.getMetalKernelArgType(&I);
  ...
  case Instruction::ShuffleVector:
    ...
    pushValue(cast<ShuffleVectorInst>(I).getShuffleMaskForBitcode(), InstID,
              Vals);
    break;
  case Instruction::ICmp: { //  CMP2: [opty, opval, opval, pred]
    Code = bitc::FUNC_CODE_INST_CMP2;
    AbbrevToUse = FUNCTION_INST_CMP_ABBREV;

    // For ICMP, both operands must have the same type
    // Use the original instruction operand types
    if (pushValueAndType(I.getOperand(0), InstID, Vals))
      AbbrevToUse = 0;
    pushValue(I.getOperand(1), InstID, Vals);
  ...
  case Instruction::FCmp: {
    ...
    Code = bitc::FUNC_CODE_INST_CMP2;
    // DXIL approach: disable abbreviations for control flow to ensure
    // compatibility
    AbbrevToUse = 0;
    pushValueAndType(I.getOperand(0), InstID, Vals);
    pushValue(I.getOperand(1), InstID, Vals);
  ...
  case Instruction::IndirectBr:
    ...
    // Use corrected Metal kernel argument types for i64→i32 conversions
    Vals.push_back(VE.getTypeID(VE.getMetalKernelArgType(I.getOperand(0))));
  ...
  case Instruction::PHI: {
    ...
    // Use corrected Metal kernel argument types for i64→i32 conversions
    Vals64.push_back(VE.getTypeID(VE.getMetalKernelArgType(&PN)));
  ...
  case Instruction::LandingPad: {
    ...
    // Use corrected Metal kernel argument types for i64→i32 conversions
    Vals.push_back(VE.getTypeID(VE.getMetalKernelArgType(&LP)));
  ...
  case Instruction::Load:
    ...
    // Use corrected Metal kernel argument types for i64→i32 conversions
    Vals.push_back(VE.getTypeID(VE.getMetalKernelArgType(&I)));
  ...
  case Instruction::VAArg:
    ...
    // Use corrected Metal kernel argument types for i64→i32 conversions
    Vals.push_back(
        VE.getTypeID(VE.getMetalKernelArgType(I.getOperand(0)))); // valistty
    pushValue(I.getOperand(0), InstID, Vals);                     // valist.
    // Also use corrected type for result type
    Vals.push_back(VE.getTypeID(VE.getMetalKernelArgType(&I))); // restype.
    break;
...
/// Write a GlobalValue VST to the module. The purpose of this data structure
/// is to allow clients to efficiently find the function body.
...
void ModuleBitcodeWriter::writeBlockInfo() {
  ...
  }
  {
    auto Abbv = std::make_shared<BitCodeAbbrev>();
    Abbv->Add(BitCodeAbbrevOp(bitc::FUNC_CODE_INST_BR));
    // TODO: Use different abbrev for absolute value references (succ0,
    // succ1)?
    Abbv->Add(ValAbbrevOp); // succ0
...
// Unused constant commented out to suppress warnings
...
void ModuleBitcodeWriter::writePerModuleGlobalValueSummary() {
  ...
  (void)FSCallsRelBFAbbrev; // Suppress unused variable warning
  ...
  Abbv->Add(BitCodeAbbrevOp(BitCodeAbbrevOp::Array));
  Abbv->Add(BitCodeAbbrevOp(BitCodeAbbrevOp::VBR, 8));
  Stream.EmitAbbrev(std::move(Abbv)); // CallsiteAbbrev not used
  ...
  Abbv->Add(BitCodeAbbrevOp(BitCodeAbbrevOp::Array));
  Abbv->Add(BitCodeAbbrevOp(BitCodeAbbrevOp::VBR, 8));
  Stream.EmitAbbrev(std::move(Abbv)); // AllocAbbrev not used
  ...
  (void)RadixAbbrev; // Suppress unused variable warning
...
void IndexBitcodeWriter::writeCombinedGlobalValueSummary() {
  ...
  Abbv->Add(BitCodeAbbrevOp(BitCodeAbbrevOp::VBR, 8));
  unsigned FSCallsAbbrev = Stream.EmitAbbrev(std::move(Abbv));
  (void)FSCallsAbbrev; // Suppress unused variable warning
  ...
  (void)CallsiteAbbrev; // Suppress unused variable warning

  Abbv = std::make_shared<BitCodeAbbrev>();
  Abbv->Add(BitCodeAbbrevOp(bitc::FS_COMBINED_ALLOC_INFO_NO_CONTEXT));
  Abbv->Add(BitCodeAbbrevOp(BitCodeAbbrevOp::VBR, 4)); // nummib
  Abbv->Add(BitCodeAbbrevOp(BitCodeAbbrevOp::VBR, 4)); // numver
  ...
  (void)AllocAbbrev; // Suppress unused variable warning
...
static void emitDarwinBCHeaderAndTrailer(SmallVectorImpl<char> &Buffer,
  ...
  // armv[0-9]-*, thumbv[0-9]-*, armv5te-*, or armv6t2-*. The CPUType is a
  // magic number from /usr/include/mach/machine.h.  It is ok to reproduce the
  // specific constants here because they are implicitly part of the Darwin
  // ABI.
  enum {
...
void BitcodeWriter17::writeSymtab() {
  ...
  // asm parser for the target so that we can create an accurate symbol table
  // for the module.
  for (Module *M : Mods) {
  ...
  // The irsymtab::build function may be unable to create a symbol table if
  // the module is malformed (e.g. it contains an invalid alias). Writing a
  // symbol table is not required for correctness, but we still want to be
  // able to write malformed modules to bitcode files, so swallow the error.
...
void BitcodeWriter17::writeModule(const Module &M,
  ...
  // requires that the module is materialized, so we can cast to non-const
  // here, after checking that it is in fact materialized.
...
void BitcodeWriter17::writeIndex(const ModuleSummaryIndex *Index,
  ...
  IndexBitcodeWriter IndexWriter(*Stream, StrtabBuilder, *Index,
                                 TypedModuleToSummariesForIndex);
  IndexWriter.write();
}
