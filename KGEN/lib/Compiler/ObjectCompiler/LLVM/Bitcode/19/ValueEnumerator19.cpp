...
//===- ValueEnumerator19.cpp - Number values and types for bitcode writer
//---===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the ValueEnumerator19 class.
//
//===----------------------------------------------------------------------===//

#include "ValueEnumerator19.h"
...
static OrderMap orderModule(const Module &M) {
  ...
  // ValueEnumerator19::ValueEnumerator19() and
  // ValueEnumerator19::incorporateFunction().
  OrderMap OM;

  // Initializers of GlobalValues are processed in
  // BitcodeReader::ResolveGlobalAndAliasInits().  Match the order there rather
  // than ValueEnumerator19, and match the code in
  ...
  for (const Function &F : M) {
    ...
    // ValueEnumerator19::incorporateFunction() and WriteFunction().  Basic
...
ValueEnumerator19::ValueEnumerator19(const Module &M,
  ...
  //
  // TODO: Move this to ValueEnumerator19::EnumerateOperandType() once bitcode
...
unsigned ValueEnumerator19::getInstructionID(const Instruction *Inst) const {
  InstructionMapType::const_iterator I = InstructionMap.find(Inst);
  assert(I != InstructionMap.end() && "Instruction is not mapped!");
  return I->second;
}

unsigned ValueEnumerator19::getComdatID(const Comdat *C) const {
  unsigned ComdatID = Comdats.idFor(C);
  assert(ComdatID && "Comdat not found!");
  return ComdatID;
}

void ValueEnumerator19::setInstructionID(const Instruction *I) {
  InstructionMap[I] = InstructionCount++;
}

unsigned ValueEnumerator19::getValueID(const Value *V) const {
...
LLVM_DUMP_METHOD void ValueEnumerator19::dump() const {
...
void ValueEnumerator19::print(raw_ostream &OS, const ValueMapType &Map,
...
void ValueEnumerator19::print(raw_ostream &OS, const MetadataMapType &Map,
                              const char *Name) const {
  OS << "Map Name: " << Name << "\n";
  OS << "Size: " << Map.size() << "\n";
  for (const auto &I : Map) {
...
void ValueEnumerator19::OptimizeConstants(unsigned CstStart, unsigned CstEnd) {
...
void ValueEnumerator19::EnumerateValueSymbolTable(const ValueSymbolTable &VST) {
...
/// module.
void ValueEnumerator19::EnumerateNamedMetadata(const Module &M) {
  for (const auto &I : M.named_metadata())
    EnumerateNamedMDNode(&I);
}

void ValueEnumerator19::EnumerateNamedMDNode(const NamedMDNode *MD) {
  for (const MDNode *N : MD->operands())
    EnumerateMetadata(nullptr, N);
}

unsigned ValueEnumerator19::getMetadataFunctionID(const Function *F) const {
  return F ? getValueID(F) + 1 : 0;
}

void ValueEnumerator19::EnumerateMetadata(const Function *F,
                                          const Metadata *MD) {
  EnumerateMetadata(getMetadataFunctionID(F), MD);
}

void ValueEnumerator19::EnumerateFunctionLocalMetadata(
    const Function &F, const LocalAsMetadata *Local) {
  EnumerateFunctionLocalMetadata(getMetadataFunctionID(&F), Local);
}

void ValueEnumerator19::EnumerateFunctionLocalListMetadata(
    const Function &F, const DIArgList *ArgList) {
  EnumerateFunctionLocalListMetadata(getMetadataFunctionID(&F), ArgList);
}

void ValueEnumerator19::dropFunctionFromMetadata(
...
void ValueEnumerator19::EnumerateMetadata(unsigned F, const Metadata *MD) {
...
const MDNode *ValueEnumerator19::enumerateMetadataImpl(unsigned F,
                                                       const Metadata *MD) {
  if (!MD)
    return nullptr;
...
void ValueEnumerator19::organizeMetadata() {
...
void ValueEnumerator19::incorporateFunctionMetadata(const Function &F) {
...
void ValueEnumerator19::EnumerateValue(const Value *V) {
...
void ValueEnumerator19::EnumerateType(Type *Ty) {
...
void ValueEnumerator19::EnumerateOperandType(const Value *V) {
  EnumerateType(V->getType());
...
void ValueEnumerator19::EnumerateAttributes(AttributeList PAL) {
  if (PAL.isEmpty())
...
void ValueEnumerator19::incorporateFunction(const Function &F) {
  InstructionCount = 0;
...
void ValueEnumerator19::purgeFunction() {
...
unsigned ValueEnumerator19::getGlobalBasicBlockID(const BasicBlock *BB) const {
...
uint64_t ValueEnumerator19::computeBitsRequiredForTypeIndices() const {
