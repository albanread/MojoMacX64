...
static OrderMap orderModule(const Module &M) {
  ...
  // ValueEnumerator21::ValueEnumerator21() and
  // ValueEnumerator21::incorporateFunction().
  OrderMap OM;

  // Initializers of GlobalValues are processed in
  // BitcodeReader::ResolveGlobalAndAliasInits().  Match the order there rather
  // than ValueEnumerator21, and match the code in
  ...
  for (const Function &F : M) {
    ...
    // ValueEnumerator21::incorporateFunction() and WriteFunction().  Basic
...
ValueEnumerator21::ValueEnumerator21(const Module &M,
  ...
  //
  // TODO: Move this to ValueEnumerator21::EnumerateOperandType() once bitcode
...
unsigned ValueEnumerator21::getInstructionID(const Instruction *Inst) const {
  InstructionMapType::const_iterator I = InstructionMap.find(Inst);
  assert(I != InstructionMap.end() && "Instruction is not mapped!");
  return I->second;
}

unsigned ValueEnumerator21::getComdatID(const Comdat *C) const {
  unsigned ComdatID = Comdats.idFor(C);
  assert(ComdatID && "Comdat not found!");
  return ComdatID;
}

void ValueEnumerator21::setInstructionID(const Instruction *I) {
  InstructionMap[I] = InstructionCount++;
}

unsigned ValueEnumerator21::getValueID(const Value *V) const {
...
LLVM_DUMP_METHOD void ValueEnumerator21::dump() const {
...
void ValueEnumerator21::print(raw_ostream &OS, const ValueMapType &Map,
...
void ValueEnumerator21::print(raw_ostream &OS, const MetadataMapType &Map,
                              const char *Name) const {
  OS << "Map Name: " << Name << "\n";
  OS << "Size: " << Map.size() << "\n";
  for (const auto &I : Map) {
...
void ValueEnumerator21::OptimizeConstants(unsigned CstStart, unsigned CstEnd) {
...
void ValueEnumerator21::EnumerateValueSymbolTable(const ValueSymbolTable &VST) {
...
/// module.
void ValueEnumerator21::EnumerateNamedMetadata(const Module &M) {
  for (const auto &I : M.named_metadata())
    EnumerateNamedMDNode(&I);
}

void ValueEnumerator21::EnumerateNamedMDNode(const NamedMDNode *MD) {
  for (const MDNode *N : MD->operands())
    EnumerateMetadata(nullptr, N);
}

unsigned ValueEnumerator21::getMetadataFunctionID(const Function *F) const {
  return F ? getValueID(F) + 1 : 0;
}

void ValueEnumerator21::EnumerateMetadata(const Function *F,
                                          const Metadata *MD) {
  EnumerateMetadata(getMetadataFunctionID(F), MD);
}

void ValueEnumerator21::EnumerateFunctionLocalMetadata(
    const Function &F, const LocalAsMetadata *Local) {
  EnumerateFunctionLocalMetadata(getMetadataFunctionID(&F), Local);
}

void ValueEnumerator21::EnumerateFunctionLocalListMetadata(
    const Function &F, const DIArgList *ArgList) {
  EnumerateFunctionLocalListMetadata(getMetadataFunctionID(&F), ArgList);
}

void ValueEnumerator21::dropFunctionFromMetadata(
...
void ValueEnumerator21::EnumerateMetadata(unsigned F, const Metadata *MD) {
...
const MDNode *ValueEnumerator21::enumerateMetadataImpl(unsigned F,
                                                       const Metadata *MD) {
  if (!MD)
    return nullptr;
...
void ValueEnumerator21::organizeMetadata() {
...
void ValueEnumerator21::incorporateFunctionMetadata(const Function &F) {
...
void ValueEnumerator21::EnumerateValue(const Value *V) {
...
void ValueEnumerator21::EnumerateType(Type *Ty) {
...
void ValueEnumerator21::EnumerateOperandType(const Value *V) {
  EnumerateType(V->getType());
...
void ValueEnumerator21::EnumerateAttributes(AttributeList PAL) {
  if (PAL.isEmpty())
...
void ValueEnumerator21::incorporateFunction(const Function &F) {
  InstructionCount = 0;
...
void ValueEnumerator21::purgeFunction() {
...
unsigned ValueEnumerator21::getGlobalBasicBlockID(const BasicBlock *BB) const {
...
uint64_t ValueEnumerator21::computeBitsRequiredForTypeIndices() const {
