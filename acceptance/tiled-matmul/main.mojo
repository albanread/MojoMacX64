...
# ===----------------------------------------------------------------------=== #
# Modular's tiled matrix multiplication (max/examples/gpu-block-and-warp),
# carried into this fork's collection unmodified. The strongest GPU proof
# in the set: shared-memory tiles, barrier synchronisation between load and
# compute phases, and a validation pass -- all through the AIR backend.
