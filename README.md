...
| [**MojoCocoa**](https://github.com/albanread/MojoCocoa) | macOS ARM64<br/>Apple Silicon | Apple M4 Max, 32-core GPU<br/>Mac Studio (4096 fp32 ALUs) | Mojo → AIR → Metal,<br/>via `AppleGPURT` | **Newest.** The Cocoa compiler hook and `std.objc` pass 9 of 9 spikes and the example apps build; the GPU stack now compiles and runs — a tiled shared-memory matmul at 1187 GFLOP/s on 2048³, numerically exact on every shape, recorded in the shared `oracles` repo |
